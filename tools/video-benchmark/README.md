...
The registry stores SHA-256 identities and constraints, never raw video. Before a candidate benchmark: verify source bytes, create direct human gold evidence, lock one component/model/version/configuration, run bounded tests, then retain compact receipts/artifacts outside Git. `UNTESTED — TARGET HARDWARE EXECUTION REQUIRED` is neither a pass nor a rejection.
